...
function generateAndSendTranscript(data)
    ...
    if file then
        ...
        file:close()
        debugPrint('^2[tk_report]^0 Transcript saved to:', transcriptPath)
    else
        debugPrint('^1[tk_report]^0 Failed to save transcript')
    end
    ...
    PerformHttpRequest(Config.Discord.Webhook, function(err, text, headers) 
        if err == 204 or err == 200 then
            debugPrint('^2[tk_report]^0 Discord transcript file sent successfully')
        else
            debugPrint('^1[tk_report]^0 Failed to send transcript file. Status:', err)
            debugPrint('^1[tk_report]^0 Response:', text)
        end
    end, 'POST', payload, { 
...
function sendDiscordWebhook(type, data)
    debugPrint('^3[tk_report]^0 Sending Discord webhook:', type)
        ...
        embed = {
            {
                ...
                footer = {
                    text = 'tk_report • New report created',
    ...
    elseif type == 'deleteReport' then
            ...
            embeds = {
                {
                    ...
                    footer = {
                        text = 'tk_report • Full transcript attached below',
                        icon_url = Config.Discord.ImageUrl
                    },
                    timestamp = os.date('!%Y-%m-%dT%H:%M:%SZ')
                }
        ...
        PerformHttpRequest(Config.Discord.Webhook, function(err, text, headers) 
            if err == 204 or err == 200 then
                debugPrint('^2[tk_report]^0 Discord embed sent successfully')
                generateAndSendTranscript(data)
            else
                debugPrint('^1[tk_report]^0 Failed to send Discord embed. Status:', err)
                debugPrint('^1[tk_report]^0 Response:', text)
            end
    ...
    if #embed > 0 then
        PerformHttpRequest(Config.Discord.Webhook, function(err, text, headers) 
            if err == 204 or err == 200 then
                debugPrint('^2[tk_report]^0 Discord webhook sent successfully')
            else
                debugPrint('^1[tk_report]^0 Failed to send Discord webhook. Status:', err)
            end
