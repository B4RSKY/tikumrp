...
MySQL.ready(function()
    ...
    print('[tk_report] Database tables initialized successfully')
end)

-- Events
RegisterNetEvent('tk_report:sendAdminNotification', function(data)
...
-- Callbacks
-- Create a new report
lib.callback.register('tk_report:createReport', function(source, data)
    ...
    if id then
        debugPrint('^2[tk_report]^7 Created new report with ID: ' .. id)
        TriggerClientEvent('tk_report:client:refreshReports', -1)
        TriggerEvent('tk_report:sendAdminNotification', {
        ...
        debugPrint('^1[tk_report]^7 Failed to create report')
        return { success = false }
    end
end)

-- Get reports for a specific player or all reports for admin
lib.callback.register('tk_report:getReports', function(source, filter)
    local Player = GetPlayerPed(source)
...
lib.callback.register('tk_report:updateReport', function(source, reportId, updates)
    local Player = GetPlayerPed(source)
    if not Player then return { success = false } end
...
lib.callback.register('tk_report:deleteReport', function(source, data)
    local Player = GetPlayerPed(source)
    if not Player then return { success = false } end
    ...
    if result.affectedRows > 0 then
        ...
        if Config.FiveManage.DeleteMedia then
            ...
            TriggerClientEvent('tk_report:client:deleteReportMedia', -1, mediaData)
        end
...
lib.callback.register('tk_report:addMessage', function(source, data)
    local Player = GetPlayerPed(source)
    local license = data.license or GetPlayerIdentifier(source, 0)
    local playerName = data.name or GetPlayerName(source)
    if not Player then return { success = false } end    
    ...
    if not reportExists then
        debugPrint('^1[tk_report]^0 Report ID does not exist:', data.report_id)
        ...
        for _, result in ipairs(results) do
            if result.player_id then
                ...
                if playerId ~= 0 and license ~= result.player_id then
                    local menuState = lib.callback.await('tk_report:client:menuState', source)
                ...
                if players ~= nil then
                    for _, player in ipairs(players) do
                        if IsPlayerAnAce(player.id) or player.license == result.player_id then
                            TriggerClientEvent('tk_report:client:refreshReports', player.id)
                            TriggerClientEvent('tk_report:client:refreshMessages', player.id, data.report_id)
                        end
                    end
                end
            end
...
lib.callback.register('tk_report:getReportMessages', function(source, reportId)
    local Player = GetPlayerPed(source)
    local license = GetPlayerIdentifier(source, 0)
...
lib.callback.register('tk_report:getPermissions', function(source)
    local Player = GetPlayerPed(source)
    if not Player then return {} end
    ...
    debugPrint('^3[tk_report]^0 Player Permissions:', json.encode(permissions, {indent = true}))
    
    return permissions
end)

-- Get user preferences
lib.callback.register('tk_report:getUserPreferences', function(source)
    local license = GetPlayerIdentifier(source, 0)
    if not license then 
        return { theme = 'light' } 
    end
...
lib.callback.register('tk_report:updateUserPreferences', function(source, data)
    local license = GetPlayerIdentifier(source, 0)
    if not license then 
        return { success = false } 
    end
...
lib.callback.register('tk_report:getPlayerLicenses', function(source, data)
