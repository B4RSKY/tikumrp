...
Framework.SetPlayerJob = function(src, name, grade)
    ...
    if not ESX.DoesJobExist(name, grade) then
        print(('Framework.SetPlayerJob: Invalid job %s or grade %s for player %s'):format(name, grade, src))
        return
    end
...
lib.callback.register('chat:callback:getFrameworkJobs', function(source)
