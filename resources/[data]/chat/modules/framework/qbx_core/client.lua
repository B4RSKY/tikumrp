...
Framework.ShowHelpText = function(message, _)
    return print("chat:WARN: ShowHelpText is not implemented for this framework, please set the helptext module to hide the help text.")
end

---This will hide the help text message on the screen
---@return nil
Framework.HideHelpText = function()
    return print("chat:WARN: HideHelpText is not implemented for this framework, please set the helptext module to hide the help text.")
end
...
Framework.GetItemCount = function(item)
    ...
    return 0, print("chat:WARN: GetItemCount is not implemented for this framework, please use the inventory module to get the item count. If you are using a diffrent inventory please let us know so we can bridge it and have less nonsense.")
end
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    Wait(1500)
    TriggerEvent('chat:client:onPlayerLoaded')
end)

RegisterNetEvent('QBCore:Client:OnPlayerUnload', function()
    TriggerEvent('chat:client:onPlayerUnload')
end)

RegisterNetEvent('QBCore:Client:OnJobUpdate', function(data)
    TriggerEvent('chat:client:onPlayerJobUpdate', data.name, data.label, data.grade.name, data.grade.level)
end)
