...
CreateThread(function()
    ...
    local keybind = lib.addKeybind({
        ...
        onPressed = function(self)
            if Check(cache.ped) then SoundPlay("Open") SetCursorLocation(Config.GUI.Position.x, Config.GUI.Position.y) end
            MenuOpened = true
            while MenuOpened do
                DrawGUI()
                Wait(0)
        ...
        onReleased = function(self)
            if Check(cache.ped) then MenuOpened = false SoundPlay("Close") end
        end,
    })
end)