	...
	if context == 'server' and dir == 'config.lua' then
		...
		if configFile then
			...
			if firstLine == 'Config = {}' then
				error("^3Your ^1fmid_utils^3 config is outdated, redownload the config and update it to the latest changes.^7")
			end
		end
	end
...
local version = LoadResourceFile("fmid_utils", "version")
if version then
	Utils.Version = Utils.Math.trim(version)
else
	error("^1[fmid_utils] Warning: Could not load the version file.^7")
end