...
function SetKvp(Name, Data, SetData)
    ...
    SetResourceKvp('tk_admin-'..Name, json.encode(Config.ResourceKVPs[KvpId]['Value']))
    Wait(100)
    RefreshMenu(Name)
end
...
function ResetMenuKvp()
    for i = 1, #Config.ResourceKVPs do
        SetResourceKvp("tk_admin-" .. Config.ResourceKVPs[i]['Name'], "[]")
...
function RefreshMenu(Type)
    ...
    if Type == 'all' then -- Generate All
        for i=1, #Config.ResourceKVPs do
            ...
            if GetResourceKvpString("tk_admin-"..KVP['Name']) == nil or GetResourceKvpString("tk_admin-"..KVP['Name']) == "[]" then
                KVP['Value'] = GenerateKVP(KVP['Name'])
                SetResourceKvp("tk_admin-"..KVP['Name'], json.encode(KVP['Value']))
            else
                KVP['Value'] = json.decode(GetResourceKvpString("tk_admin-"..KVP['Name']))
            end
        end
...
function CreateLog(Type, Log, Data)
    ...
    QBCore.Functions.TriggerCallback('tk_admin/server/create-log', function(result)
    end, Type, Log, Data)
end
        ...
        for i = 1, #Config.Reports do
            ...
            if Report['Id'] == ReportId then
                SetTimeout(500, function()
                    ...
                    TriggerServerEvent('tk_admin/server/sync-chat-data', 'Reports', Config.Reports, 1500)
                end)
...
function GetServerConvar(Convar)
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback('tk_admin/server/get-convar', function(ReturnData)
        Prom:resolve(ReturnData)
    end, Convar)
    return Citizen.Await(Prom)
end
...
function GetPlayersInArea(Coords, Radius)
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback('tk_admin/server/get-active-players-in-radius', function(Players)
...
function GetBans()
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback("tk_admin/server/get-bans", function(Bans)
        Prom:resolve(Bans)
    end)
    return Citizen.Await(Prom)
end
...
function GetPlayers()
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback("tk_admin/server/get-players", function(Players)
        Prom:resolve(Players)
    end)
    return Citizen.Await(Prom)
end
...
function GetLogs()
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback("tk_admin/server/get-logs", function(Logs)
        Prom:resolve(Logs)
    end)
    return Citizen.Await(Prom)
end
