        ...
        if player ~= "OFFLINE" then
            ...
            if citizenid ~= nil then
                ...
                if result[1] then
                    ...
                    if not online then
                        MySQL.insert("INSERT INTO bansapanel (name, license, discord, ip, reason, expire, bannedby) VALUES (?, ?, ?, ?, ?, ?, ?)", {
                            result[1].name,
                            result[1].license,
                            "",
                            "",
                        ...
                        TriggerEvent("qb-log:server:CreateLog", "bansapanel", "Player Banned", "red", string.format("%s was banned by %s for %s (%s hours)", result[1].name, GetPlayerName(src), reason, time / 60 / 60), Config.TagEveryone)
                    end
                else
...
RegisterServerEvent("919-admin:server:SetWeather", function(weatherType)
    ...
    if AdminPanel.HasPermission(src, "weather") then
        if QBCore then
            exports["kiz-dev"]:setWeather(weatherType)
...
RegisterServerEvent("919-admin:server:SetTime", function(hour, minute)
    ...
    if AdminPanel.HasPermission(src, "time") then
        if QBCore then
            exports["kiz-dev"]:setTime(hour, 0)
...
RegisterServerEvent("919-admin:server:UnbanPlayer", function(license)
    ...
    if AdminPanel.HasPermission(src, "unban") then
        MySQL.query("SELECT * FROM `bansapanel` WHERE license = ? LIMIT 1", {license}, function(result)
            if result[1] then
                MySQL.query("DELETE FROM `bansapanel` WHERE license = ? LIMIT 1", {license}, function(rowsAffected)
                    if rowsAffected then
                    ...
                    local results = MySQL.query.await("SELECT * FROM `bansapanel`")
                    local BansInfo = {}
