...
RegisterNetEvent("919-admin:server:ReportReply", function(data)
    ...
    if data.name ~= nil then
        for k, Player in ipairs(QBCore.Functions.GetQBPlayers()) do
            local correctname = GetPlayerName(v).." ("..Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname..")"
...
RegisterNetEvent("inventory:server:OpenInventory", function(target)
    local playerId = tonumber(target)
    if playerId then
        exports.ox_inventory:openInventory('player', playerId)
    end
