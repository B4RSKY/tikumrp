...
function HasItem(items, amount)
    if items and amount then
        local count = exports.ox_inventory:Search('count', items:lower())
        if type(count) == 'table' then
            local newCount = count[items:upper()]
            if newCount >= amount then return true end
        else
...
function QBCore.Functions.Notify(title, text, type, duration, position)
    lib.notify({
