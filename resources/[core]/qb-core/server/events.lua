...
AddEventHandler('playerDropped', function(reason)
    ...
    local message = '**'..name..'** has left the server \n **Reason:** `'..reason..'`\n\n**Player: '..name..'**\n`🔢` **Server ID:** `'..src..'`\n`🎮` **Steam Hex:** `'..identifier..'`'
    TriggerEvent('qb-log:server:CreateLog', 'keluar', 'Player Left', 'red', message, false)
...
RegisterNetEvent('QBCore:UpdatePlayer', function()
    ...
    newHunger = tonumber(string.format("%.2f", newHunger))
    newThirst = tonumber(string.format("%.2f", newThirst))
    newStress = tonumber(string.format("%.2f", newStress))
