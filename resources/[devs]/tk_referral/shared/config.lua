...
-- === Core requirement (TEST MODE: 1 menit) ===
Config.RequirementSeconds = 120                -- ganti 3600 saat produksi
Config.Grace = { Enable = true, Minutes = 15, MaxActiveSeconds = 300 } -- boleh switch 1x

-- Referrer harus aktif harian agar muncul/menjaga skor
Config.DailyReferrerActivity = {
  Enable = true,
  Window = 'hour',         -- 'day' (default live) | 'hour' (mode test)
  Minutes = 1,             -- syarat aktif per window (TEST: 1 menit biar cepat)
  Policy = 'DECAY',        -- atau 'HIDE' kalau kamu maunya tampil/hilang
...
-- === AFK filter (server akan hitung aktif jika bergerak) ===
Config.AfkDetect = {
    Enable = true,
    MaxIdleSeconds = 15,      -- utk test-mu; production saran >= 45-60
...
-- === Leaderboard & Webhook ===
Config.Leaderboard = {
    Mode = 'lifetime',        -- hanya lifetime sesuai permintaan
    TopN = 10,
...
Config.Validation = {
    ...
    PlaytimeGate = { Enabled = false, MaxSeconds = 0 } -- kamu minta: semua player bisa redeem tanpa gate
}

-- === Hadiah ===
-- Hanya referral COMPLETED (>=RequirementSeconds) yang dihitung ke scoreboard.
Config.Rewards = {
...
Config.UI = {
    Theme = 'purple-black',
    UseNUILeaderboard = false  -- pakai ox_lib context saja
}

Config.Code = {
  Prefix = 'TIKUM-',   -- prefix kode
  RandomLength = 6     -- panjang bagian random (6–8 aman untuk VARCHAR(16))
}
