    ...
    while true do
        ...
        for id, xPlayer in pairs(players) do
            ...
            if GetPlayerName(id) then
                local playerJobName = QBCore and xPlayer.PlayerData.job.name or xPlayer.job.name
                local playerOnDuty = QBCore and xPlayer.PlayerData.job.onduty or true
                for job, tables in pairs(Config.authorizedJob) do
                    local authorized = tables.sharedjobs[playerJobName]
                    if authorized and playerOnDuty then
                        for n = 1, #data[job] do
