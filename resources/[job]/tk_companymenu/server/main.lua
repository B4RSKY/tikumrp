...
-- Get job data
QBCore.Functions.CreateCallback('tk_companymenu:server:GetJobData', function(source, cb, jobName)
    local src = source
...
function StartRefreshTimer(src, jobName)
    ...
    RefreshTimers[src] = SetTimeout(interval, function()
        ...
        if Player and Player.PlayerData.job.name == jobName then
            ...
            TriggerClientEvent('tk_companymenu:client:RefreshData', src)
...
-- Get user settings
QBCore.Functions.CreateCallback('tk_companymenu:server:GetSettings', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('tk_companymenu:server:UpdateEmployee', function(citizenid, jobName, grade)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    return
        ...
        if targetPlayer then
            ...
            if targetCurrentJob == jobName then
                ...
                targetPlayer.Functions.SetJob(jobName, grade)
                TriggerClientEvent('tk_companymenu:client:JobChanged', -1, jobName)
...
-- Hire new employee
RegisterNetEvent('tk_companymenu:server:HireEmployee', function(targetId, jobName, grade)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('tk_companymenu:server:RemoveEmployee', function(citizenid, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
-- Save user settings
RegisterNetEvent('tk_companymenu:server:SaveSettings', function(settings)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('QBCore:Server:OnJobUpdate', function(source, newJob)
    ...
    if Player then
        ...
        TriggerClientEvent('tk_companymenu:client:JobChanged', -1, newJob.name)
    end
end)
...
QBCore.Commands.Add('managejob', 'Open job management interface', {}, false, function(source)
    ...
    TriggerClientEvent('tk_companymenu:client:TriggerOpenManager', source, {jobData = jobName})
end)

RegisterNetEvent('tk_companymenu:server:RefreshPlayTime', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
function LoadSocietyTransactions()
    ...
    if result and #result > 0 then
        ...
        print("^2[tk_companymenu]^7 No society transactions found in database")
    end
end
...
QBCore.Functions.CreateCallback('tk_companymenu:server:GetSocietyData', function(source, cb, jobName)
    local src = source
...
RegisterNetEvent('tk_companymenu:server:DepositMoney', function(amount, note, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('tk_companymenu:server:WithdrawMoney', function(amount, note, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('tk_companymenu:server:TransferMoney', function(citizenid, amount, note, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('tk_companymenu:server:SubmitApplication', function(jobName, answers)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('tk_companymenu:server:GetJobApplications', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('tk_companymenu:server:UpdateApplicationStatus', function(applicationId, status, notes)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('tk_companymenu:server:CheckPendingApplication', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('tk_companymenu:server:CheckApplicationStatus', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('tk_companymenu:server:RequestRefreshJobData', function()
    TriggerEvent('QBCore:server:UpdateObject')
end)
...
QBCore.Functions.CreateCallback('tk_companymenu:server:GetEmployeePermissions', function(source, cb, citizenid, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('tk_companymenu:server:UpdateEmployeePermissions', function(source, cb, citizenid, jobName, permissions)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    MySQL.Async.execute('INSERT INTO job_employee_permissions (citizenid, job, permissions, granted_by) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE permissions = ?, granted_by = ?', 
        ...
        if rowsChanged > 0 then
            ...
            local targetPlayer = QBCore.Functions.GetPlayerByCitizenId(citizenid)
            if targetPlayer then
                TriggerClientEvent('tk_companymenu:client:RefreshPermissions', targetPlayer.PlayerData.source, permissions)
            ...
            for _, p in pairs(players) do
                if p.PlayerData.job.name == jobName then
                    TriggerClientEvent('tk_companymenu:client:SyncPermissions', p.PlayerData.source, citizenid, permissions)
                end
            end
...
QBCore.Functions.CreateCallback('tk_companymenu:server:HireNewEmployee', function(source, cb, targetId, jobName, grade)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local Target = QBCore.Functions.GetPlayer(tonumber(targetId))
...
QBCore.Functions.CreateCallback('tk_companymenu:server:HasJobAccess', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('tk_companymenu:server:PermissionAction', function(action, data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('tk_companymenu:server:GetPlaytimeData', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('tk_companymenu:server:GetJobGrades', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
